...
impl<'a> ModuleManager<'a> {
    ...
    pub fn handle_message(&self, message: CanMessage) {
        println!("Message received: id={:#08x}", message.id());
        ...
        match opcode {
            ...
            _ => println!("Unknown request {:#02x}", opcode),
        }
    }
    ...
    fn assign_module_id(&self, in_message: CanMessage) {
        ...
        out_message.set_data(1, ((in_message.id() >> 24) & 0xff) as u8);
        out_message.set_data(2, ((in_message.id() >> 16) & 0xff) as u8);
        out_message.set_data(3, ((in_message.id() >> 8) & 0xff) as u8);
        out_message.set_data(4, (in_message.id() & 0xff) as u8);
